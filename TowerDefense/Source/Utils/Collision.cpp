...
namespace Collision
{
	...
	bool PixelPerfectTest(const sf::Sprite& Object1, const sf::Sprite& Object2, sf::Uint8 AlphaLimit) {
		...
		if (Object1.getGlobalBounds().intersects(Object2.getGlobalBounds(), Intersection)) {
			...
			for (int i = ToInt(Intersection.left); i < Intersection.left+Intersection.width; i++) {
				for (int j = ToInt(Intersection.top); j < Intersection.top+Intersection.height; j++) {
 
					sf::Vector2f o1v = Object1.getInverseTransform().transformPoint(ToFloat(i), ToFloat(j));
					sf::Vector2f o2v = Object2.getInverseTransform().transformPoint(ToFloat(i), ToFloat(j));
	...
	public:
		OrientedBoundingBox (const sf::Sprite& Object) // Calculate the four points of the OBB from a transformed (scaled, rotated...) sprite
		{
			...
			Points[1] = trans.transformPoint(ToFloat(local.width), 0.f);
			Points[2] = trans.transformPoint(ToFloat(local.width), ToFloat(local.height));
			Points[3] = trans.transformPoint(0.f, ToFloat(local.height));
		}
	...
	{
		return static_cast<int>(std::trunc(Float));
	}
