...
namespace TowerDefense
{
	namespace GameEngine
	{
		...
		void SoundManager::start_music(const std::string& asset_path)
		{
			...
			GlobalShared::get_sound(asset_path)->setLoop(true); // probably won't overlap this way :/
			GlobalShared::get_sound(asset_path)->setPitch(1.f);
			GlobalShared::get_sound(asset_path)->setVolume(
				Constants::SoundsAssets::all_assets_sounds_volume_map.at(asset_path)
			);
			GlobalShared::get_sound(asset_path)->setPlayingOffset(sf::Time::Zero);
			GlobalShared::get_sound(asset_path)->play();
		}
		...
		void SoundManager::pause_music()
		{
			...
			GlobalShared::get_sound(current_music_asset_path)->pause();
		}

		void SoundManager::resume_music()
		{
			assert(current_music_asset_path != "");
			assert(GlobalShared::get_sound(current_music_asset_path)->getStatus() 
				== GlobalShared::get_sound(current_music_asset_path)->Paused);
			GlobalShared::get_sound(current_music_asset_path)->play();
		}
		...
		void SoundManager::stop_music()
		{
			...
			if(current_music_asset_path != ""
				&& !(GlobalShared::get_sound(current_music_asset_path)->getStatus() 
				== GlobalShared::get_sound(current_music_asset_path)->Playing ||
				GlobalShared::get_sound(current_music_asset_path)->getStatus() 
				== GlobalShared::get_sound(current_music_asset_path)->Paused))
			{
		...
		{
			assert(current_music_asset_path != "");
			GlobalShared::get_sound(current_music_asset_path)->setPitch(value);
		}

		void SoundManager::set_current_music_volume(const float& value)
		{
			assert(current_music_asset_path != "");
			GlobalShared::get_sound(current_music_asset_path)->setVolume(value);
		}

		void SoundManager::set_default_music_volume()
		{
			assert(current_music_asset_path != "");
			GlobalShared::get_sound(current_music_asset_path)->setVolume(
				Constants::SoundsAssets::all_assets_sounds_volume_map.at(current_music_asset_path)
			);
		}
	}
}
